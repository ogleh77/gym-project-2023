...
public class RegistrationController extends CommonClass implements Initializable {
    ...
    private Customers savingCustomer() {
        ...
        customer = new CustomerBuilder().setShift(shift.getValue()).setCustomerId(customerId).setGander(gander).setWhoAdded(activeUser.getUsername()).setImage(image).setAddress(_address).setLastName(lastName.getText()).setFirstName(firstName.getText()).setMiddleName(middleName.getText()).setPhone(phone.getText()).setWeight(_weight).build();
        return customer;
    }
